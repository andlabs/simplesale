...
func (m *Method) decl(curiface string, name string, this bool, args []string) string {
	var s string

	if m.Ret[len(m.Ret) - 1] == '*' {
		s = m.Ret[:len(m.Ret) - 1] + " *"
	} else {
		s = m.Ret + " "
	}
...
func (m *Method) VtableDecl(curiface string) string {
	return m.decl(curiface, "(*" + m.Name + ")", false, m.ArgTypes)
}

func (m *Method) GlobalDecl(curiface string) string {
	return m.decl(curiface, curiface + m.Name, false, m.ArgTypes)
}

func (m *Method) GlobalDefn(curiface string) string {
	return m.decl(curiface, curiface + m.Name, true, m.ArgsFull)
}
